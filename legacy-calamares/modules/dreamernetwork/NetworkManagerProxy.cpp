...
//
// NOTE for whoever builds this on the NUC: this talks to the *live ISO's*
// NetworkManager over the system D-Bus (org.freedesktop.NetworkManager),
// the same instance the desktop's network applet would use. It does NOT
// touch NetworkManager inside the not-yet-installed target system — that
// happens later, at install time, via the "networkcfg" exec module, using
// the ssid/psk this class connected with (stashed in GlobalStorage by
...
DreamerNetworkManagerProxy::DreamerNetworkManagerProxy( QObject* parent )
    : QObject( parent )
...
QString
DreamerNetworkManagerProxy::findWifiDevicePath() const
{
    QDBusReply<QList<QDBusObjectPath>> devices = m_nmInterface->call( "GetDevices" );
    if ( !devices.isValid() )
    {
        cWarning() << "dreamernetwork: could not enumerate NetworkManager devices:" << devices.error().message();
        return QString();
    }
    ...
    cWarning() << "dreamernetwork: no Wi-Fi device found on this machine";
    return QString();
}

void
DreamerNetworkManagerProxy::requestScan()
{
    ...
    QTimer::singleShot( 3000, this, &DreamerNetworkManagerProxy::onScanResultsReady );
}

void
DreamerNetworkManagerProxy::onScanResultsReady()
{
...
void
DreamerNetworkManagerProxy::connectToOpenNetwork( const QString& ssid )
{
    connectToSecuredNetwork( ssid, QString() );
}

void
DreamerNetworkManagerProxy::connectToSecuredNetwork( const QString& ssid, const QString& password )
{
    if ( m_wifiDevicePath.isEmpty() )
    {
        return;
...
void
DreamerNetworkManagerProxy::onNMStateChanged( uint newState )
{
